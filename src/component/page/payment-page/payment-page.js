...
const RenderRazorpay = ({
  ...
  const handlePayment = async (status, orderDetails = {}) => {
    await Axios.post('http://localhost:5000/paymentCapture',
      {
        status,
        orderDetails,
      });
  ...
  // we will be filling this object in next step.
    ...
    name: 'Kishore', // Title for your organization to display in checkout modal
    // image, // custom logo  url
    order_id: orderId, // order id from props
    callback_url: 'http://localhost:3006/register-for-session', // Your success URL
    prefill: {
      name: 'Kishore',
